...
void decodeIR(uint32_t code)
{
  ...
  if      (code > 0xFFFFFF) return; //invalid code
    ...
    switch (irEnabled) {
	  ...
	  case 8: decodeIR21TV(code);    break;
      default: return;
    }
  }
...
void decodeIR21TV(uint32_t code)
{
  switch (code) {
    case IR21TV_POWER           : toggleOnOff();  break;
	case IR21TV_BRIGHTER        : changeBrightness(16);                                       break;
	case IR21TV_DARKER          : changeBrightness(-16);                                      break;
	case IR21TV_AUTO            : effectCurrent = FX_MODE_STATIC;                             break;
    case IR21TV_FLASH           : effectCurrent = FX_MODE_PALETTE;                            break;
	case IR21TV_COLORTWINKLE    : if (!applyPreset(1)) { effectCurrent = FX_MODE_COLORTWINKLE;  effectPalette = 0; } break;
    case IR21TV_STROBE          : if (!applyPreset(2)) { effectCurrent = FX_MODE_RAINBOW_CYCLE; effectPalette = 0; } break;
    case IR21TV_FADE            : if (!applyPreset(3)) { effectCurrent = FX_MODE_BREATH;        effectPalette = 0; } break;
    case IR21TV_SMOOTH          : if (!applyPreset(4)) { effectCurrent = FX_MODE_RAINBOW;       effectPalette = 0; } break;
    case IR21TV_FADE7           : if (!applyPreset(3)) { effectCurrent = FX_MODE_FIRE_FLICKER;  effectPalette = 0; } break;
    case IR21TV_DIY5            : if (!applyPreset(5)) { effectCurrent = FX_MODE_METEOR_SMOOTH; effectPalette = 0; } break;
    case IR21TV_DIY6            : if (!applyPreset(6)) { effectCurrent = FX_MODE_RAIN;          effectPalette = 0; } break;
